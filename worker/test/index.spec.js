...
describe("Rule worker", () => {
  ...
  it("serves committed rule assets with a query token", async () => {
  ...
  it("rewrites private GitHub self-references inside modules", async () => {
    const response = await exports.default.fetch(
      "https://example.com/Surge/Module/%E7%BD%91%E6%98%93%E4%BA%91%E9%9F%B3%E4%B9%90%E5%8E%BB%E5%B9%BF%E5%91%8A%20fix@myself.sgmodule?token=test-download-token",
    );
    ...
    expect(body).toContain(
      "https://example.com/Surge/Script/netease.adblock.surge.js?token=test-download-token",
    );
    expect(response.headers.get("Cache-Control")).toBe("no-store");
  });
